...
parser.add_option('-c','--project', help='Name of customer or project. Be carefull with what you put in cause this will create a catalog using the options as the catalog name. Choosing bad project name could potentionally hurt your system. Customername becomes a catalog in /opt/werfen/result catalog. Legal characther in project name is: a-z, 0-9 and , _ - ',dest='customer')
parser.add_option('-i','--ip',help='IP address to scan. All nmap format is supported. The IP adresses should be wrapped around the " " like this: "10.1.2.3 10.4.5.6 10.7.8.0/24". ', dest='ip')

parser.add_option('-p','--port', help='Choise are: q=quick, n=normal, F=full, r=range. Quick scan will scan the host for a few TCP and UDP ports. Quick scan is like ping scan, but this options do a bit more then just ping. Normal scan is the most commond 4k plus ports. Full scan is the hole range of ports, 65535. if you put in a range of ports like this: "445-7500" then ports between 445 and 7500 will be scanned. The use of this options is like this: -p "22,21,20,25,80-88,445-1023". You may give just single ports, ranges or any nmap port options as long as you keep it within the " ".  ', dest='port' )


parser.add_option('-t','--timeout',help='Timeout value. Specify the timeout in milliseconds. If you do not specify any timeout value, then the system will use 50ms as the default time out value for you. ', dest='defaulttimeout')

parser.add_option('-P','--allhosts', action='store_true', help='This options is the same as nmap\'s P0 options and will scan all hosts regardless of which is found to be online.', dest='Pnull')

parser.add_option('-6','--ipv6',help='IPv6 scan. Do you want to scan IPv6 use this option. TO BE INNSTALLED :-)  ' )
...
parser.add_option('-v','--verbose',action='store_true',help='Be verbose, need som reading? buy a book or use this options. This will print lots of information for you.',dest='verbose')
